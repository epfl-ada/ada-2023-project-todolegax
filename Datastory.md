...
After the preprocessing, the dataset contains $42'923$ beers, $58'199$ users, $14'158$ breweries and a total of $2'587'598$ reviews, with both textual content and numerical rating.
...
By performing a linear regression analysis, we can clearly see that there is a positive linear correlation between the size metric and the average rating, with an high $R^2$ score. However, we must remain cautious with these results, as it is likely that other cofounders, not taken into account, have an impact on the average rating obtained by the breweries. It is therefore likely that the size index does not explain everything.
...
It can be seen that belonging to a larger category tends to increase the average rating obtained significantly (at significance level 0.05). An average rating difference of almost 0.4 can be observed between small and larger breweries. However, we must be careful, as the standard deviation remains quite high. 
...
For example, the 15 breweries with highest average rating are all `small` breweries. That shows that despite the fact that bigger breweries have higher rating in average, the perceived quality of their products is not as high as the best small breweries. Therefore, being small-scaled can actually be an advantage for obtaining outstanding products.
...
We can see that the difference of review length are significant at the 0.05 significance level. However, like in the rating case, it is interesting to see that the longest reviews are given to the small breweries. This shows that some small breweries have reviewers that left more sophisticated reviews than for the bigger breweries.
...
It would be interesting to see now if these results holds over the years...
...
Let's look at how its average rating and the distance of its reviewers is evolving over the years.
